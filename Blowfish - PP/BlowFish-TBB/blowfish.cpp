...
byte* BLOWFISH::Crypt_ECB(byte* data, int length, int* newlength, void (BLOWFISH::*CryptBlock)(byte*, int), bool decrypt)
{
	...
	tbb::parallel_for(int(0), *newlength, int(8), [=](size_t i){
...
void BLOWFISH::Encrypt_Block(byte* block, int offset)
{
	unsigned xr_var = 0;
	unsigned xl_var = 0;
	setblock(block, offset, &xr_var,&xl_var);
	encipher(&xr_var,&xl_var);
	getblock(block, offset,&xr_var,&xl_var);
}
...
void BLOWFISH::Decrypt_Block(byte* block, int offset)
{
	...
	unsigned xl_var = 0;
	setblock(block, offset, &xr_var,&xl_var);
	decipher( &xr_var,&xl_var);
	getblock(block, offset, &xr_var,&xl_var);
}

void BLOWFISH::setblock(byte* block, int offset,unsigned* xr,unsigned* xl)
{
	//TODO: CHECK ENDIANNESS
	*xr = 0; *xl = 0;
	for (int i = 0; i < 4; i++)
	{
		*xl = (*xl << 8) + block[offset + i];
		*xr = (*xr << 8) + block[4 + offset + i];
	}
}

void BLOWFISH::getblock(byte* block, int offset,unsigned* xr_var,unsigned* xl_var)
{
	//TODO: CHECK ENDIANNESS
	unsigned int xl = *xl_var;
	unsigned int xr = *xr_var;
	for (int i = 3; i >= 0; i--)
	{
...
void BLOWFISH::SetupKey(byte* cipherKey, int length)
{
	...
	std::string vcipher = std::string((char*)cipherKey, length);
	if (length > 56)
	{
		throw 56;
	}
...
void BLOWFISH::encipher(unsigned* xr_var,unsigned* xl_var)
{
	*xl_var ^= p[0];
	for (int i = 0; i < ROUNDS; i += 2)
	{
		*xr_var = round(*xr_var, *xl_var, i + 1);
		*xl_var = round(*xl_var, *xr_var, i + 2);
	}
	*xr_var ^= p[ROUNDS + 1];

	unsigned int swap = *xl_var;
	*xl_var = *xr_var;
	*xr_var = swap;
}

void BLOWFISH::decipher(unsigned* xr_var,unsigned* xl_var)
{
	*xl_var ^= p[ROUNDS + 1];
	for (int i = ROUNDS; i > 0; i -= 2)
	{
		*xr_var = round(*xr_var, *xl_var, i);
		*xl_var = round(*xl_var, *xr_var, i - 1);
	}
	*xr_var ^= p[0];
